...
|HTTP_CLIENT_MAX_CONNECTIONS_COUNT|cpu_count * 2| Http client is shared between BqClient, BqStorageApi, GCStorage clients. There are workloads, like Parallel Export, that will try to open one read and one write http connection per cpu. Plus some logic could use additional connection for getting service information, like info about table or status of query execution. <br /> Recommended way to calculate pool size:<br /> connections_per_request = cpus_count * (1 read or write connection + 1 service connection) <br /> connections_per_request = cpus_count * 2; <br /> requests_count = 1 // for mainframe client size <br /> requests_count = number or expected jobs run in parallel // for server side<br />  HTTP_CLIENT_MAX_CONNECTIONS_COUNT = requests_count * connections_per_request|

...
object MergeInto extends Logging {
  def run(bq: BigQuery, source: TableId, target: TableId, naturalKeyColumns: Seq[String], creds: GoogleCredentials, dryRun: Boolean): Unit = {
    val mergeRequest = buildMergeRequest(bq, source, target, naturalKeyColumns)

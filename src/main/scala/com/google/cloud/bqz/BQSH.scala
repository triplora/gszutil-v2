...
import com.google.cloud.bqz.cmd.{Load, Mk, Query, Result, Rm}
...
object BQSH {
  def main(args: Array[String]): Unit = {
    val mbr = args.headOption.getOrElse("")
    val script = CrossPlatform.readDDString(mbr)
    val env = Map.empty[String,String]
    run(script, env)
  }

  def run(script: String, env: Map[String,String] = Map.empty): Result = {
    val endEnv = splitSH(script)
      .map(readArgs)
      .foldLeft(env){(a,b) =>
        val result = exec(b, a)
        a ++ result.export
      }
    Result(export = endEnv)
  }
  ...
  def exec(args: Seq[String], env: Map[String,String] = Map.empty): Result = {
    lazy val fail = Result.Failure(s"invalid command '${args.mkString(" ")}'")

    ShParser.parse(args, env) match {
      case Some(cmd) =>
        if (cmd.name == "bq"){
